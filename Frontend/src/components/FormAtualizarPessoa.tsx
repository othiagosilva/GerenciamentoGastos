...
export function FormAtualizarPessoa({aoSucesso }: any) {
  ...
  const salvar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resposta = await api.put(`/Pessoa/${idPessoa}`, payload);
      const id = resposta.data.id || resposta.data.idPessoa || null;
      setIdPessoa(id);
      setMensagemSucesso("Pessoa alterada com sucesso!");
      aoSucesso();
    } catch (err: any) {
  ...
  return (
    <form onSubmit={salvar} className="bg-gray-800 p-8 rounded-2xl border border-gray-700 shadow-2xl max-w-2xl mx-auto space-y-4">
      <h2 className="text-2xl font-bold mb-4 text-blue-600">Atualizar Pessoa</h2>    
      ...
      <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-bold text-lg transition-all active:scale-95 shadow-lg">
        Atualizar Pessoa
      </button>
    </form>
  );
